...
node {
    ...
    ws("${gopath}/src/github.com/ONSdigital/dp-dd-csv-filter") {
        stage('Checkout') {
            checkout scm
            sh 'git clean -dfx'
        ...
        stage('Build') {
            sh "GOPATH=${gopath} go build -o build/dp-dd-csv-filter"
        }
        ...
        stage('Image') {
            docker.withRegistry("https://${env.ECR_REPOSITORY_URI}", { ->
                ...
                docker.build('dp-dd-csv-filter', '--no-cache --pull --rm .').push(revision)
            })
        }
        ...
        stage('Bundle') {
            ...
            sh "tar -cvzf dp-dd-csv-filter-${revision}.tar.gz appspec.yml scripts/codedeploy"
        ...
        stage('Deploy') {
            sh sprintf('aws deploy create-deployment %s %s %s,bundleType=tgz,key=%s', [
                ...
                "dp-dd-csv-filter-${revision}.tar.gz",
            ])
        }
    }
}
